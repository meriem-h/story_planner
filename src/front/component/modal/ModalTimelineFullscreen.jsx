...
export default function TimelineFullscreen({ selectedTome, chapters }) {
    const api = useApi()
    ...
    const filteredChapters = filterChapter === 'all'
        ? chapters
        : chapters.filter(ch => ch.id == filterChapter)

    const renderItem = (item, big = false) => (
        <div key={item.id} className="relative flex flex-col items-center gap-1 cursor-grab">
            ...
            <div
                ...
                className={`rounded-full border-2 hover:scale-110 transition-transform cursor-pointer ${big ? 'w-8 h-8' : 'w-5 h-5'} ${getBubbleClass(item)}`}
                onClick={(e) => { e.stopPropagation(); setOpenPopover(openPopover === item.id ? null : item.id) }}
            />
            <span className={`text-orange-500 whitespace-nowrap truncate text-center font-medium ${big ? 'text-sm max-w-[100px]' : 'text-xs max-w-[70px]'}`}>
                {item.title}
            </span>
        </div>
    )

    const renderSortable = (list, chapterId, big = false) => (
        <ReactSortable
            list={list}
            ...
            group="timeline-fs"
            className="flex items-center flex-wrap gap-2 min-h-[40px] min-w-[40px]"
        >
            {list.map((item, i) => (
                <React.Fragment key={item.id}>
                    {renderItem(item, big)}
                    {i < list.length - 1 && (
                        <div className={`h-[2px] bg-orange-300 shrink-0 ${big ? 'w-8' : 'w-5'}`} />
                    )}
                </React.Fragment>
            ))}
        </ReactSortable>
    ...
    return (
        <div className="flex flex-col h-full gap-4">
            ...
            {/* Header filtre + bouton ajouter */}
            <div className="flex items-center justify-between gap-2">
                <div className="flex gap-2 flex-wrap">
                    <button
                        onClick={() => setFilterChapter('all')}
                        className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${filterChapter === 'all' ? 'bg-orange-400 text-white' : 'bg-orange-100 text-orange-400 hover:bg-orange-200'}`}
                    >
                        Tous
                    </button>
                    {chapters.map(ch => (
                        <button
                            ...
                            className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${filterChapter == ch.id ? 'bg-orange-400 text-white' : 'bg-orange-100 text-orange-400 hover:bg-orange-200'}`}
                        >
                            {ch.title}
                        </button>
                    ))}
            ...
            {/* Corps : non placés à gauche + timeline à droite */}
            <div className="flex gap-4 flex-1 overflow-hidden">
                        ...
                        {unplaced.map(item => (
                            <div key={item.id} className="flex items-center gap-2 cursor-grab">
                                <div
                                    ...
                                    onClick={(e) => { e.stopPropagation(); setOpenPopover(openPopover === item.id ? null : item.id) }}
                                />
                                <span className="text-xs text-orange-500 truncate">{item.title}</span>
                            </div>
                        ))}
                    </ReactSortable>
                </div>
                ...
                <div className="flex-1 min-w-0 overflow-x-auto">
                    <div className="flex items-start py-2 gap-0" style={{ width: 'max-content' }}>
                        {filteredChapters.map((chapter) => (
                            <div key={chapter.id} className="flex flex-col items-center shrink-0 border-s border-orange-400 px-4">
                                <span className="text-sm text-orange-500 font-semibold mb-3 whitespace-nowrap">{chapter.title}</span>
                                {renderSortable(grouped[chapter.id] || [], chapter.id, true)}
                            </div>
                        ))}
                    </div>
                </div>
