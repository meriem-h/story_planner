...
import { Menu, X, Lightbulb, BookUser, BookOpenText, NotebookPen, ScrollText } from 'lucide-react'
        ...
        <div className="flex h-screen">
            ...
            <div className={`${isOpen ? 'w-80 px-4' : 'w-0'} transition-all duration-300 overflow-hidden bg-orange-200`}>
                <div className='p-4 flex gap-2 justify-center'>
                    <button className='bg-orange-400 rounded-2xl p-2 text-orange-100' title="Chapitres"
                        onClick={() => setContent(<Chapter chapters={chapters} selectedChapter={selectedChapter} setSelectedChapter={setSelectedChapter} books={books} selectedBook={selectedBook} setSelectedBook={setSelectedBook} addChapter={addChapter} addBook={addBook} />)}>
                        <BookOpenText />
                    </button>
                    <button className='bg-orange-400 rounded-2xl p-2 text-orange-100' title="Personnages" onClick={() => setContent(<Character selectedBook={selectedBook} />)}><BookUser /></button>
                    <button className='bg-orange-400 rounded-2xl p-2 text-orange-100' title="Lore" onClick={() => setContent(<Lore selectedBook={selectedBook} />)}><ScrollText /></button>
                    <button className='bg-orange-400 rounded-2xl p-2 text-orange-100' title="Snippets" onClick={() => setContent(<Snippet selectedBook={selectedBook} />)}><Lightbulb /></button>
                    <button className='bg-orange-400 rounded-2xl p-2 text-orange-100' title="Notes" onClick={() => setContent(<Note selectedBook={selectedBook} />)}><NotebookPen /></button>
